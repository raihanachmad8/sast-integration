    ...
    <aside style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRight: 'none', background: token.colorSidebarBgDark, color: token.colorSidebarTextDark, zIndex: 60, transition: 'width 200ms ease, flex 200ms ease', ...(isMobile ? { position: 'fixed' as const, top: 0, left: 0, height: '100vh', width: mobileOpen ? 260 : 0, flex: mobileOpen ? '0 0 260px' : '0 0 0', transform: mobileOpen ? 'translateX(0)' : 'translateX(-100%)' } : { position: 'relative' as const, width: 260, flex: '0 0 260px' }) }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: token.paddingSM, padding: `${token.paddingMD}px ${token.paddingMD}px ${token.paddingLG}px`, borderBottom: `1px solid ${token.colorSidebarBorderDark}`, flexShrink: 0 }}>
        ...
        <div style={{ minWidth: 0, lineHeight: 1.15 }}>
          <div style={{ overflow: 'hidden', color: token.colorTextLightSolid, fontSize: token.fontSize, fontWeight: token.fontWeightStrong, textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>SAST Workspace</div>
          <div style={{ marginTop: token.marginXXS, overflow: 'hidden', color: token.colorSidebarTextSecondaryDark, fontSize: token.fontSizeSM, fontWeight: token.fontWeightStrong, textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>AI-assisted security review</div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        {navSections.map((section, si) => (
          <div style={{ padding: `${token.paddingMD}px ${token.paddingSM}px`, ...(si > 0 ? { borderTop: `1px solid ${token.colorSidebarBorderDark}` } : {}) }} key={section.title}>
            <div style={{ margin: `0 ${token.paddingSM}px ${token.paddingXS}px`, color: token.colorSidebarTextSecondaryDark, fontSize: token.fontSizeSM, fontWeight: token.fontWeightStrong, letterSpacing: '0.16em', textTransform: 'uppercase' as const }}>{section.title}</div>
            {section.groups.map((group, gi) => (
              <div style={{ marginTop: token.marginSM }} key={group.title ?? `${section.title}-${gi}`}>
                {group.title && <div style={{ margin: `${token.marginSM}px ${token.paddingSM}px ${token.paddingXXS}px`, color: token.colorSidebarTextSecondaryDark, fontSize: token.fontSizeSM, fontWeight: token.fontWeightStrong, letterSpacing: '0.12em', textTransform: 'uppercase' as const }}>{group.title}</div>}
                <nav style={{ display: 'grid', gap: token.marginXXS }} aria-label={group.title ?? section.title}>

...
export async function GET(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const jobs = await knowledgeBackfillService.listJobs(workspaceId, sourceId);
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    if (payload && typeof payload === 'object' && 'resume' in payload && (payload as { resume: boolean }).resume) {
      const job = await knowledgeBackfillService.resume(workspaceId, sourceId);
      logger.knowledge.info('resumeBackfill completed', { jobId: job.id });
      return ApiResponse.success('Knowledge backfill resumed', job);
    }

    const job = await knowledgeBackfillService.start(workspaceId, sourceId, payload);

...
/**
 * Summary cards showing running, done, and failed scan counts with icons.
 *
 * Derives counts from the provided scan rows and displays them as styled stat cards.
 *
 * @param props - {@link ScanSummaryCardsProps}
 * @returns JSX element rendering a row of scan status summary cards.
 *
 * @example
 * <ScanSummaryCards scans={scanRows} />
 */
export function ScanSummaryCards({ scans }: ScanSummaryCardsProps) {
  ...
  const running = scans.filter(s => s.status === SCAN_STATUS.RUNNING).length;
  const completed = scans.filter(s => s.status === SCAN_STATUS.COMPLETED).length;
  const failed = scans.filter(s => s.status === SCAN_STATUS.FAILED).length;
  const managed = scans.filter(s => s.origin === SCAN_ORIGIN.MANAGED).length;
  const external = scans.filter(s => s.origin === SCAN_ORIGIN.EXTERNAL_UPLOAD).length;
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            {STATUS_ITEMS.map((item, i) => (
              <Flex key={item.key} align="center" gap={token.marginXS}>
                <div style={{ width: token.sizeMS, height: token.sizeMS, borderRadius: token.borderRadiusSM, background: item.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>

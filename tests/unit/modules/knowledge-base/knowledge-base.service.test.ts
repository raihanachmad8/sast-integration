...
describe('knowledgeBaseService.listByWorkspace', () => {
  ...
  it('+ should return paginated entries', async () => {
    mockRepo.countEntries.mockResolvedValue(1);
    mockRepo.listEntries.mockResolvedValue([{ id: 'kb-1', title: 'XSS Fix' }]);
  ...
  it('+ should apply custom pagination', async () => {
    ...
    expect(mockRepo.listEntries).toHaveBeenCalledWith(expect.objectContaining({ limit: 10, offset: 10 }));
  });
  ...
  it('+ should apply search and source filters', async () => {
    ...
    expect(mockRepo.listEntries).toHaveBeenCalledWith(expect.objectContaining({ search: 'XSS', source: 'src-1' }));
  });
...
describe('knowledgeBaseService.getById', () => {
  ...
  /**
   * Purpose: Validates that an entry is returned when found
   */
  it('+ should return entry when found', async () => {
    mockRepo.findEntryById.mockResolvedValue({ id: 'kb-1', title: 'XSS Fix' });
  ...
  it('- should throw NOT_FOUND when entry does not exist', async () => {
    mockRepo.findEntryById.mockResolvedValue(null);
    await expect(knowledgeBaseService.getById('ws-1', 'kb-1')).rejects.toThrow('Knowledge entry not found');
  });

  /**
   * Purpose: Validates that the repository is called with correct entryId
   */
  it('- should call repo with entryId', async () => {
    mockRepo.findEntryById.mockResolvedValue(null);
    await knowledgeBaseService.getById('ws-2', 'kb-2').catch(() => {});
    expect(mockRepo.findEntryById).toHaveBeenCalledWith('kb-2');
  });
});
...
describe('knowledgeBaseService.createEntry', () => {
  ...
  it('+ should update source entry count after creation', async () => {
    ...
    expect(mockRepo.updateSourceEntryCount).toHaveBeenCalledWith('src-1', 5, expect.anything());
  });

  /**
   * Purpose: Validates that non-members cannot create knowledge entries
   */
  ...
  it('- should throw NOT_FOUND when source does not exist in workspace', async () => {
    mockWorkspaceRepo.getMemberRole.mockResolvedValue('member');
    mockRepo.findSourceById.mockResolvedValue(null);
...
describe('knowledgeBaseService.updateEntry', () => {
  ...
  /**
   * Purpose: Validates that non-members cannot update knowledge entries
   */
  it('- should throw FORBIDDEN when user is not a member', async () => {
    mockWorkspaceRepo.getMemberRole.mockResolvedValue(null);
  ...
  /**
   * Purpose: Validates that updating a nonexistent entry throws NOT_FOUND
   */
  it('- should throw NOT_FOUND when entry does not exist', async () => {
    mockWorkspaceRepo.getMemberRole.mockResolvedValue('member');
    mockRepo.findEntryById.mockResolvedValue(null);
...
describe('knowledgeBaseService.deleteEntry', () => {
  ...
  /**
   * Purpose: Validates that non-members cannot delete knowledge entries
   */
  it('- should throw FORBIDDEN when user is not a member', async () => {
    mockWorkspaceRepo.getMemberRole.mockResolvedValue(null);
  ...
  /**
   * Purpose: Validates that deleting a nonexistent entry throws NOT_FOUND
   */
  it('- should throw NOT_FOUND when entry does not exist', async () => {
    mockWorkspaceRepo.getMemberRole.mockResolvedValue('member');
    mockRepo.findEntryById.mockResolvedValue(null);
...
describe('knowledgeBaseService.muteEntry', () => {
  ...
  /**
   * Purpose: Validates that muting a nonexistent entry throws NOT_FOUND
   */
  it('- should throw NOT_FOUND when entry does not exist', async () => {
    mockWorkspaceRepo.getMemberRole.mockResolvedValue('member');
    mockRepo.findEntryById.mockResolvedValue(null);
...
describe('knowledgeBaseService edge cases', () => {
  ...
  it('+ should handle large page numbers', async () => {
    mockRepo.countEntries.mockResolvedValue(100);
    mockRepo.listEntries.mockResolvedValue([]);

    await knowledgeBaseService.listByWorkspace('ws-1', { page: 100, perPage: 10 });
    expect(mockRepo.listEntries).toHaveBeenCalledWith(expect.objectContaining({ offset: 990 }));
  });
  ...
  it('+ should handle special characters in search', async () => {
    ...
    expect(mockRepo.listEntries).toHaveBeenCalled();
  });
   ...
   */
  it('+ should call listEntries with default params', async () => {
    mockRepo.countEntries.mockResolvedValue(0);
    mockRepo.listEntries.mockResolvedValue([]);

    await knowledgeBaseService.listByWorkspace('ws-1');
    expect(mockRepo.listEntries).toHaveBeenCalledWith(expect.objectContaining({ limit: 25, offset: 0 }));
  });
});
